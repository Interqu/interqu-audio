...
```bash
sam-lambda$ sam local invoke InferenceFunction --event events/event.json
```
...
sam-lambda$ curl http://localhost:3000/classify_digit
```
...
```bash
sam-lambda$ sam logs -n InferenceFunction --stack-name "sam-lambda" --tail
```
